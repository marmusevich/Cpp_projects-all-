...
bool cOutResult::createDir(void)
{
    ...
    if(access(Dir.c_str(), 00) == -1) //если нет католога создаём
#if defined (_WIN32) || defined (WIN32)
    //код WIN
    ...
    if(access(RepDir.c_str(), 00) == -1) //если нет католога создаём
#if defined (_WIN32) || defined (WIN32)
    //код WIN
        if(mkdir(RepDir.c_str())==-1)
#else
    //код LINUX
...
bool cOutResult::createTmpDir(void)
{
    ...
    if(access(tmpDir.c_str(), 00) == -1) //если нет католога создаём
#if defined (_WIN32) || defined (WIN32)
    //код WIN
