...
public:
    ...
    : socket_(std::move(socket))
  {
    ...
    std::cout << "My IP according to google is: " << socket_.local_endpoint().address().to_string() << std::endl;
  ...
  void do_read()
  {
    ...
    socket_.async_read_some(boost::asio::buffer(data_, max_length),
        [this, self](boost::system::error_code ec, std::size_t length)
        {
          if (!ec)
          {
            // получить адрес удаленный
            std::cout << "read from IP: " << socket_.remote_endpoint().address().to_string() <<" mess is: ";
